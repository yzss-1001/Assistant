...
def identify_image():
    data = request.get_json()
    image = data.get('image', 'Guest')
    # print(f"图像信息：{image}")
    img = methods.base64_to_bgr(image)
    img = methods.crop_to_24x24(img)
    predict_data = methods.change_to_hist(img)
    result = methods.predict_photo(predict_data)
    print(result)
    return jsonify({'success': True, 'identify_result': result})
