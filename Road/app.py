    ...
    with open('txt/user/user_information', 'r+') as file:  # 检查登录信息在user_information中是否存在
        ...
        for i in sub_body:  # 遍历每一个信息体
            if login_user_body in i:  # 判断该登录信息体是否在user_information中存在
                ...
                return jsonify({'success': True, 'message': '登录成功！', 'user': username, 'email': email})
...
def register():  # 注册信息后台处理
    ...
    print(f"注册：{password}")
    print(f"注册：{email}")
    with open('txt/user/user_information', 'r+') as file:  # 将注册信息存入user_information中，同时避免用户名信息重复。
        existing_content = file.read()
        existing_count = existing_content.count(';;')
        if username not in existing_content:
            file.writelines(f"\nid:{existing_count + 1}\nusername:{username}\npassword:{password}\nemail:{email};;")
